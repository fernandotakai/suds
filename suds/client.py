...
class Client(object):
    ...
    def __init__(self, url_or_file, **kwargs):
        """
        ...
        self.wsdl = reader.open(url_or_file)
...
class Method:
    ...
    def processtransporterror(self, e): 
        """ 
        Processes a transport error, be it generated by the transport or simulated 
        during the processing of an async reply 
        @param e: A transport error object 
        @type: TransportError 
        @return: None or the return value from self.failed 
        """ 
    ...
    def processreply(self, reply, **kwargs): 
        """ 
        Process a reply asynchrounously 
        @param reply: A reply object 
        @type: suds.transport.Reply 
        @return: processed reply delivering the same results as if the method was called synchrounoulsy 
           including exceptions if HTTP responses other than 200 are passed to the method 
        """ 

        result = None
        binding = self.method.binding.input
        retxml = self.client.options.retxml
        ...
        if reply.code == 200:
            ...
            log.error('Transport error detected when processing the asynchronous reply') 
            e = TransportError('', reply.code, cStringIO.StringIO(reply.message))
            result = self.processtransporterror(e) 
    ...
    def succeeded(self, binding, reply):
        ...
        plugins = PluginContainer(self.client.options.plugins)
        if len(reply) > 0:
